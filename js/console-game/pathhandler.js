...
(function (root) {
  Josh.PathHandler = function (shell, config) {
    ...
    var _shell = shell;
    _shell.templates.not_found = (obj) => {
      return "<div>" + obj.cmd + ": " + obj.path + ": No such file or directory</div>";
    };
    _shell.templates.ls = (obj) => {
      var res = "<div>";
      for (let i = 0; i < obj.nodes.length; i++) {
        res += "<span>" + obj.nodes[i].name + "&nbsp;</span>";
      }
      res += "</div>";
      return res;
    };
    _shell.templates.pwd = (obj) => "<div>" + obj.node.path + "&nbsp;</div>";
    _shell.templates.prompt = (obj) => obj.node.path + " $";
    ...
    function completeChildren(node, partial, callback) {
      self.getChildNodes(node, function (childNodes) {
        callback(_shell.bestMatch(partial, childNodes.map(function (x) {
          return x.name;
        })));
      });
    }
